class policy:
    ...
    def calcLevelOneData(self):
        ...
        self.derivedData['Part9Tier'] = 'A' if part9ForAllVehicles == 'Yes' else 'B'
        ...
        result = lookup_TierTable(AccountTier=self.derivedData['AccountTier'], YearsInsuredTier=self.derivedData['YearsInsuredTier'], ContinuouslyTier=self.derivedData['ContinuouslyTier'],
                                  MultiCarTier=self.derivedData['MultiCarTier'], LeastFavorableTier=self.derivedData['LeastFavorableTier'], Part9Tier=self.derivedData['Part9Tier'])
        ...
        # if this is wrong, uncomment below -- update 9/9/2022 yeah, multicar here counts everything
...
class driver:
    ...
    def calcLevelOneData(self):
        ...
        self.derivedData['VehicularHomicideRiskFactor'] = self.inputs['VehicularHomicide'] == 'Yes' * lookup_
    ...
    def calcLevelTwoData(self):
        ...
        # logic build from Licenced_Class
        if self.policy.derivedData['RatingPlan'] == 'A':
            if 0 <= self.inputs['YearsLicensed'] and self.inputs['YearsLicensed'] <= 5:
                self.derivedData['YearsLicensedClass'] = str(
                    self.inputs['YearsLicensed'])  # note string here
        ...
        for vehicle in policy.vehicles:
            if vehicle.inputs['AutoUsedInBusiness'] == 'Yes' and self.inputs['YearsLicensed'] > 5:
                ...
                result = lookup_ClassATable(operatorType=self.inputs['OperatorType'][vehicle.vehicleId], licencedClass=self.derivedData['YearsLicensedClass'],
                                            driverAge=self.derivedData['DriverAgeClass'], vehicleBusiness=vehicle.inputs['AutoUsedInBusiness'], driverTraining=self.inputs['DriverTraining'])
                ...
                result = lookup_ClassBTable(operatorType=self.inputs['OperatorType'][vehicle.vehicleId], licencedClass=self.derivedData['YearsLicensedClass'],
                                            driverAge=self.derivedData['DriverAgeClass'], vehicleBusiness=vehicle.inputs['AutoUsedInBusiness'], driverTraining=self.inputs['DriverTraining'])
                self.derivedData['OperatorClass'][vehicle.vehicleId] = result['OperatorClass']
...
class vehicle:
    ...
    def totalPremium(self):
        ...
        totalPremium += self.derivedData['excessElectronicEquipmentPremium'] + \
            self.derivedData['autoLoanLeaseGapCoveragePremium']
        return round(totalPremium, 0)
    ...
    def calcLevelOneData(self, policy):
        ...
        # all numbers here look like policy defaults?  it looks like this is "coverage costs $4 per $100 over base $1000 coverages"
        self.derivedData['excessElectronicEquipmentPremium'] = (
            self.inputs['excessEquipmentCoverage'] - 1000) / 100 * 4

        # the $25 is probably a policy default.
        self.derivedData['autoLoanLeaseGapCoveragePremium'] = 25 if self.inputs['AutoLoanLeaseGapCoverage'] == 'Yes' else 0

        # model year and mileage
        # it is possible to buy a 2023 model year car in calendar year 2022, so cut off at 0
        if self.inputs['useOfOtherAutos'] == 'Yes' or self.policy.inputs['PolicyEffectiveYear'] - self.inputs['ModelYear'] <= 0:
            ...
            self.derivedData['VehicleAge'] = str(
                self.policy['PolicyEffectiveYear'] - self.inputs['ModelYear'])
        ...
        if self.inputs['useOfOtherAutos'] == 'Yes' or self.inputs['ModelYear'] >= 2020:
            ...
            self.derivedData['ModelYearClass'] = '1989 & Prior'
        elif 1990 <= self.inputs['ModelYear'] and self.inputs['ModelYear'] <= 2008:
            # the reason for the string
            self.derivedData['ModelYearClass'] = '1990-2008'
        else:
    ...
    def calcLevelTwoData(self, policy):
        ...
        for driver in self.policy.drivers:
...
class premiumPart:
    ...
    # =IF($E$39="Trailer"
            # ,0
            # ,VLOOKUP(E43,IF(E16="A",Part3ABase_tbl,Part3BBase_tbl),2,FALSE)
            # )
    ...
    # =IF($E$39="Trailer"
            ...
            # ,IF(E45="None"
                # ,0
                # ,VLOOKUP($E$64,IF($E$16="A",Part5ABase_tbl,Part5BBase_tbl),MATCH(INDEX($E$28:$H$28,MATCH($E$5,$E$20:$H$20)),IF($E$16="A",BaseA_row,BaseB_row),0),FALSE)
                # )
            # )
    ...
    # =IF($E$39="Trailer"
            ...
            # ,IF(E46="None"
                    # ,0
                    # ,VLOOKUP(E46,IF(E16="A",Part6ABase_tbl,Part6BBase_tbl),2,FALSE)
                    # )
            # )
    ...
    # =ROUND(IF($E$48="Yes"
                # ,IF($E$39="Trailer"
                        # ,0.5*IF($E$16="A",0.11,0.06)*VLOOKUP(1,IF($E$16="A",Part7ABase_tbl,Part7BBase_tbl),MATCH(IF($E$16="A",53,10),IF($E$16="A",BaseA_row,BaseB_row),0),FALSE)
                        # ,IF($E$16="A",0.11,0.06)*VLOOKUP($E$64,IF($E$16="A",Part7ABase_tbl,Part7BBase_tbl),MATCH(INDEX($E$28:$H$28,MATCH($E$5,$E$20:$H$20)),IF($E$16="A",BaseA_row,BaseB_row),0),FALSE)
                        # )
                # ,0
                # )
            # ,0
    ...
    # =IF($E$39="Trailer"
            ...
            # ,IF(E50="None"
                    # ,0
                    # ,VLOOKUP(F16,SubTrans_tbl,MATCH(E50,SubTrans_row,0),FALSE)
                    # )
            # )
    ...
    # =IF($E$39="Trailer"
        ...
        # ,IF(E51="None"
                # ,0
                # ,VLOOKUP(F16,TowingLabor_tbl,MATCH(E51,TowingLabor_row),FALSE)
                # )
        # )
    ...
    # =IF($E$39="Trailer"
        ...
        # ,IF(E52="None"
                # ,0
                # ,VLOOKUP(E52,IF(E16="A",Part12ABase_tbl,Part12BBase_tbl),2,FALSE)
                # )
        # )
    ...
    def baseRate(self):
        ...
        hasMabi = False if self.vehicle.coverages['Part5'] == 'None' else True
        hasMpay = False if self.vehicle.coverages['Part6'] == 'None' else True
        hasRent = False if self.vehicle.coverages['Part10'] == 'None' else True
        hasTowl = False if self.vehicle.coverages['Part11'] == 'None' else True
        hasUdbi = False if self.vehicle.coverages['Part12'] == 'None' else True
        hasCollision = True if self.vehicle.coverages['Part7'] == 'Yes' else False
        hasLimitedCollision = True if self.vehicle.coverages['Part8'] == 'Yes' else False
        ...
        if isTrailer:
            if self.partType in (1, 2, 3, 4, 5, 6, 8, 10, 11, 12):
                return 0
            elif (self.partType == 7 and hasCollision):
                baseRateResult = lookup_BaseRateTable(
                    part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=1, operatorClass=53 if self.policy.derivedData['RatingPlan'] == 'A' else 10)
                return round(baseRateResult['rate'] * 0.5, 0)
            elif (self.partType == 8 and hasLimitedCollision):
                baseRateResult = lookup_BaseRateTable(
                    part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=1, operatorClass=53 if self.policy.derivedData['RatingPlan'] == 'A' else 10)
                limitedMult = .11 if self.policy.derivedData['RatingPlan'] == 'A' else .06
                return round(baseRateResult['rate'] * 0.5 * limitedMult, 0)
            elif (self.partType == 9 and hasComprehensive):
                baseRateResult = lookup_BaseRateTable(
                    part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=1, operatorClass=53 if self.policy.derivedData['RatingPlan'] == 'A' else 10)
                return round(baseRateResult['rate'] * 0.5, 0)
            else:
            ...
            if self.partType in (1, 2, 4) or (self.partType == 5 and hasMabi):
                baseRateResult = lookup_BaseRateTable(part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=self.vehicle.inputs[
                                                      'Territory'], operatorClass=self.vehicle.assignedDriver.derivedData['OperatorClass'][self.vehicle.vehicleId])
                return baseRateResult['rate']
            elif self.partType == 3:
                baseRateResult = lookup_Part3BaseRateTable(
                    ratingPlan=self.policy.derivedData['RatingPlan'], limit=self.vehicle.coverages['Part3'])
                return baseRateResult['rate']
            elif (self.partType == 6 and hasMpay):
                baseRateResult = lookup_Part6BaseRateTable(
                    ratingPlan=self.policy.derivedData['RatingPlan'], limit=self.vehicle.coverages['Part6'])
                return baseRateResult['rate']
            elif (self.partType == 7 and hasCollision):
                baseRateResult = lookup_BaseRateTable(part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=self.vehicle.inputs[
                                                      'Territory'], operatorClass=self.vehicle.assignedDriver.derivedData['OperatorClass'][self.vehicle.vehicleId])
                pickupMult = .6 if isPickup else 1
                return round(baseRateResult['rate'] * pickupMult, 0)
            elif (self.partType == 8 and hasLimitedCollision):
                baseRateResult = lookup_BaseRateTable(part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=self.vehicle.inputs[
                                                      'Territory'], operatorClass=self.vehicle.assignedDriver.derivedData['OperatorClass'][self.vehicle.vehicleId])
                limitedMult = .11 if self.policy.derivedData['RatingPlan'] == 'A' else .06
                return round(baseRateResult['rate'] * limitedMult, 0)
            elif (self.partType == 9 and hasComprehensive):
                baseRateResult = lookup_BaseRateTable(part=self.partType, ratingPlan=self.policy.derivedData['RatingPlan'], territory=self.vehicle.inputs[
                                                      'Territory'], operatorClass=self.vehicle.assignedDriver.derivedData['OperatorClass'][self.vehicle.vehicleId])
                pickupMult = .9 if isPickup else 1
                return round(baseRateResult['rate'] * pickupMult, 0)
            elif (self.partType == 10 and hasRent):
                baseRateResult = lookup_SubTransTable(
                    ratingPlan=self.policy.derivedData['RatingPlan'])
                return baseRateResult[self.vehicle.coverages['Part10']]
            elif (self.partType == 1 and hasTowl):
                baseRateResult = lookup_TowingLaborTable(
                    ratingPlan=self.policy.derivedData['RatingPlan'])
                return baseRateResult[self.vehicle.coverages['Part11']]
            elif (self.partType == 12 and hasUdbi):
                baseRateResult = lookup_Part12BaseRateTable(
                    ratingPlan=self.policy.derivedData['RatingPlan'], coverage=self.vehicle.coverages['Part12'])
                return baseRateResult['rate']
            else:
                return 0

    def adjustedBaseRate(self):
        if self.partType in (7, 8) and self.vehicle.inputs['StatedAmountCoverage'] == 'Yes':
            baseRate = self.baseRate()
            collisionSymbol = 70 if self.vehicle.inputs[
                'CollisionSymbol'] > 70 else self.vehicle.inputs['CollisionSymbol']
            lookupSymbol = 6 if self.vehicle.inputs['useOfOtherAutos'] == 'Yes' else collisionSymbol
            statedAmountDivsor = lookup_StatedAmountTable(symbol=lookupSymbol)

            return round((baseRate / statedAmountDivsor['divisor']) * (self.vehicle.input['InsuredValue'] / 100), 2)
        elif self.partType == 9 and (self.vehicle.inputs['StatedAmountCoverage'] == 'Yes' or self.vehicle.inputs['AgreedAmountCoverage'] == 'Yes'):
            baseRate = self.baseRate()
            comprehensiveSymbol = 70 if self.vehicle.inputs[
                'ComprehensiveSymbol'] > 70 else self.vehicle.inputs['ComprehensiveSymbol']
            lookupSymbol = 6 if self.vehicle.inputs['useOfOtherAutos'] == 'Yes' else comprehensiveSymbol
            statedAmountDivsor = lookup_StatedAmountTable(symbol=lookupSymbol)
            agreedAmountMult = 1.1 if self.vehicle.inputs['AgreedAmountCoverage'] == 'Yes' else 1

            return round((baseRate / statedAmountDivsor['divisor']) * (self.vehicle.input['InsuredValue'] / 100), 2) * agreedAmountMult
        else:
            return self.baseRate()

    def subTotalAfterTierFactor(self):
        if self.partType in (1, 2, 3, 4, 5, 6, 7, 8, 9, 12):
            return self.roundingByPart(self.adjustedBaseRate()*self.policy.derivedData['TierFactor'])
        else:
            self.adjustedBaseRate()

    def subTotalAfterYearsLicensedFactor(self):
        if self.partType in (1, 2, 3, 4, 5, 6, 7, 8, 9, 12):
            yearsLicensedFactor = lookup_YearsLicensedTable(
                yearsLicensed=self.vehicle.assignedDriver.inputs['YearsLicensed'])

            self.derivedData['YearsLicensedFactor'] = yearsLicensedFactor['factor'] if self.policy.derivedData['RatingPlan'] == 'A' else 1

            return self.roundingByPart(self.subTotalAfterTierFactor()*self.derivedData['YearsLicensedFactor'])
        else:
            self.subTotalAfterTierFactor()

    # The above is row 143 in the Excel
    # this starts rows 144 - 171
    def subTotalAfterRiskFactors(self):
        # first we calc and store all the risk factors
        vehicleAgeFactor = lookup_VehicleAgeTable(
            vehicleAge=self.vehicle.derivedData['VehicleAge'])
        self.derivedData['VehicleAgeRSF'] = vehicleAgeFactor['RiskScoreFactor']

        # note, this is not years licensed Factor above
        ylFactor = lookup_YearsLicensedRSFTable(
            yearsLicensed=self.vehicle.assignedDriver.inputs['YearsLicensed'])
        self.derivedData['YearsLicensedRSF'] = ylFactor['RiskScoreFactor']

    # This takes over at row 172 in the excel. 144-171 is in progress
    # subTotalAfterRiskRating refers to row 171

    def subTotalBeforeDiscounts(self):
        if self.vehicle.inputs['useOfOtherAutos'] == 'No' and self.vehicle.inputs['namedNonOwnerPolicy'] == 'No':
            return None  # this whole item does not apply and is not used further
        else:
            if self.partType in (2, 7, 8, 9, 10, 11):
                self.derivedData['NonOwnerFactor'] = 0
            elif self.partType in (3, 12):
                self.derivedData['NonOwnerFactor'] = 1
            else:
                nonOwner = lookup_nonOwnerTable(
                    NonOwner=self.vehicle.inputs['namedNonOwnerPolicy'], Exclusion=self.vehicle.inputs['ExclusionApplies'], Family=self.vehicle.inputs['WithFamily'])
                self.derivedData['NonOwnerFactor'] = nonOwner['factor']

            if self.partType in (10, 11):
                self.derivedData['OtherAutosFactor'] = 0
            elif self.partType in (1, 2, 3, 7, 8, 9, 12):
                self.derivedData['OtherAutosFactor'] = 1
            else:
                otherAutos = lookup_OtherAutosTable(OtherAutos=self.vehicle.inputs['useOfOtherAutos'], PublicLivery=self.vehicle.inputs['PublicLiveryConveyances'], Primary=self.vehicle.inputs[
                                                    'IsTherePrimaryInsurance'], HHmembers=self.vehicle.inputs['WithHouseholdMembers'], Business=self.vehicle.inputs['SellingServicingRepairingParking'])


)               self.derivedData['OtherAutosFactor']=outerAutos['factor']

            if self.vehicle.inputs['namedNonOwnerPolicy'] == 'Yes'
                return self.subTotalAfterRiskRating() * self.derivedData['NonOwnerFactor']

            if self.vehicle.inputs['useOfOtherAutos'] == 'Yes':
                return self.subTotalAfterRiskRating() * self.derivedData['OtherAutosFactor']

    def subTotalAfterMileage(self):
        if self.partType in (1, 2, 3, 4, 5, 6, 7, 8, 12):
            if self.vehicle.inputs['Antique'] == 'Yes':
                discount=1
            else:
                yearsLicensedClass=self.vehicle.assignedDriver.derivedData[
                    'AnnualMileageYL'] if self.policy.derivedData['RatingPlan'] == 'A' else 'All'
                mileageDiscount= lookup_AnnualMileageTable(mileageClass = self.vehicle.derivedData['AnnualMileageClass']
                                                            , yearsLicensedClass = yearsLicensedClass
                                                            , ratingPlan = self.policy.derivedData['RatingPlan'])
                discount=mileageDiscount['discount']
            self.derivedData['MileageDiscount']=discount
            return self.roundingByPart(self.subTotalAfterRiskRating() * discount)
        else:
            return self.subTotalAfterRiskRating()

    def subTotalAfterMultiCar(self):
        if self.PartType in (1, 2, 4, 5, 7, 8, 9):
            if self.vehicle.inputs['Antique'] == 'Yes':
                discount=1
            else:
                carCount=len(self.policy.vehicles) if len(
                    self.policy.vehicles) <= 2 else 2
                multiCar=lookup_MultiCarTable(
                    carCount = carCount, ratingPlan = self.policy.derivedData['RatingPlan'])

                discount=multiCar['discount']
            self.derivedData['MultiCarDiscount']=discount
            return self.roundingByPart(self.subTotalAfterMileage() * discount)
        else:
            return self.subTotalAfterMileage()

    def subTotalAfterAntiTheft(self):
        if self.partType == 9:
            if self.policy.derivedData['RatingPlan'] == 'A':
                result=lookup_AntiTheftTable(
                    antiTheftType = self.vehicle.inputs['AntiTheftDevice'])
    ...
    def basicPackageAdjFactor(self):
        ...
        if self.partType in (1, 2, 3, 4, 12):
            if self.vehicle.derivedData['BasicPlanApplied'] == 'Yes':
                yearsLicensedRSF=self.derivedData['YearsLicensedRSF'] if self.derivedData['YearsLicensedRSF'] > 1 else 1
                garagingTown='All'
                tier='All'
                operatorClass=self.vehicle.assignedDriver.derivedData[
                    'OperatorClass'][self.vehicle.vehicleId]
    ...
    def subTotalRatingPlanB(self):
        if self.policy.derivedData['RatingPlan'] == 'B':
            ...
            raise InvalidRatingPlan(
                'subTotalRatingPlanB is only valid for rating plan B')
